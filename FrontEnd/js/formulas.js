...
$(document).ready(function () {
  ...
  function logistic(event) {
    ...
    if (
      ...
      $("#sample_size_error").text("*مقادیر ورودی باید مثبت و معتبر باشند!");
      $("#sample_size_success").text(``);
    } else {
      $("input[name=sample_size]").val(sample_size);
      $("#sample_size_success").text(
  ...
  function avg1(event) {
    ...
    if (
      ...
      const zAlpha = parseFloat(distribution.ppf(1 - 0.05 / 2).toFixed(5));
      ...
      (parseFloat(sigma1Value) <= 0 && parseFloat(sigma1Value) % 1 == 0) ||
      (parseFloat(sigma2Value) <= 0 && parseFloat(sigma2Value) % 1 == 0) ||
      ...
      const zBetha = parseFloat(distribution.ppf(1 - bethaValue).toFixed(5));
